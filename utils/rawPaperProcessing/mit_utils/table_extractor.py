...
# FIX 1: Update gensim import for newer versions
try:
    from gensim.models import KeyedVectors as keyedvectors
except ImportError:
    try:
        from gensim.models.keyedvectors import KeyedVectors as keyedvectors
    except ImportError:
...
# FIX 2: Update sklearn import for newer versions  
try:
    import joblib
except ImportError:
...
class TableExtractor(object):
    ...
    def load_embeddings(self, file_loc=None):
        if file_loc == None:
            ...
            try:
                # Try newer gensim API first
                self.embeddings = keyedvectors.load(file_loc)
            except AttributeError:
                # Fallback for older gensim versions
                from gensim.models.deprecated import keyedvectors as old_kv
                self.embeddings = old_kv.KeyedVectors.load(file_loc)
            
            # Handle both old and new gensim vocabulary access
            try:
                # New gensim API
                vocab_items = [(k, v) for k, v in self.embeddings.key_to_index.items()]
                vectors = self.embeddings.vectors
            except AttributeError:
                # Old gensim API
                vocab_items = [(k, v) for k, v in self.embeddings.vocab.items()]
                vectors = self.embeddings.syn0
            
            self.emb_vocab_ft = dict([('<null>', 0), ('<oov>', 1)] +
                         [(k, i+2) for i, (k, v) in enumerate(vocab_items)])
            self.emb_weights_ft = np.vstack([np.zeros((1,100)), np.ones((1,100)), vectors])
